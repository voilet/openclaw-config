...
DEFAULT_CLAUDE = os.environ.get("CLAUDE_CODE_BIN", "/opt/homebrew/bin/claude")
...
def run_with_pty(cmd: list[str], cwd: str | None) -> int:
    ...
    # macOS script command has different syntax than Linux
    if platform.system() == "Darwin":
        # macOS: script -q file command args...
        proc = subprocess.run([script_bin, "-q", "/dev/null"] + cmd, cwd=cwd, text=True)
    else:
